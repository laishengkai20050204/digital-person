...
2. Spring 轮询器抢占到期人物的数据库租约。
3. 任务运行期间定期续租，并受整体决策 deadline 约束。
4. Java 自动结算旧状态效果。
5. Java 自动调用活动决策模型。
6. 如有新活动，Java 自动调用状态效果评估模型。
7. 所有模型入口共享全局并发上限。
8. 所有步骤按时成功后，以乐观锁统一保存人物聚合。
9. 调度表保存新的 `nextReviewAt`，等待下一轮。

调度器默认关闭，只有设置 `ACTIVITY_SCHEDULER_ENABLED=true` 后才会产生自动模型调用。

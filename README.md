...
1. 增加实时事件开始、结束和历史补录 HTTP 接口
2. 确保所有事件写入口只调用 `PersonEventCommandService`
3. 收紧 `Person` 中可绕过应用服务的直接事件修改方法
4. 为 Agent 增加工具数量、超时、结果大小、幂等性和并行安全限制
5. 接入 Mem0 长期记忆和聊天历史持久化

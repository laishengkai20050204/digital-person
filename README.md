...
`LLM_MODEL` 必须使用 OpenRouter 模型标识，例如供应商和模型组成的 slug。API Key 不会被配置对象或日志输出。
...
日志只记录 `PersonId`、`EventId`、模型名、服务端主机、活动类型、渠道、时间、文本长度和数量等结构化信息。禁止记录聊天正文、事件标题、地点、参与者、备注、提示词、API Key、内部测试令牌和长期记忆内容。
...
1. 在服务器 systemd 环境中配置 OpenRouter 密钥并执行一次连接测试
2. 使用 `LanguageModelGateway` 实现 `StateTransitionEvaluator`
3. 增加聊天应用服务和消息历史接口
